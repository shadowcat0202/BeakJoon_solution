...
#include <algorithm>
#define endl "\n"
#define Q9465_MAX 100000

int Q9465_bf[2][Q9465_MAX] = { 0 };
int Q9465_dp[2][Q9465_MAX] = { 0 };
...
int Q9465_solution() {
	...
	for (int i = 0; i < 2; i++) {
		for (int j = 0; j < N; j++) {
			std::cin >> Q9465_bf[i][j];
		}
	}

	Q9465_dp[0][0] = Q9465_bf[0][0];
	Q9465_dp[1][0] = Q9465_bf[1][0];
	Q9465_dp[0][1] = Q9465_bf[0][1] + Q9465_dp[1][0];
	Q9465_dp[1][1] = Q9465_bf[1][1] + Q9465_dp[0][0];

	for (int i = 2; i < N; i++) {
		Q9465_dp[0][i] = Q9465_bf[0][i] + std::max(Q9465_dp[1][i - 1], Q9465_dp[1][i - 2]);
		Q9465_dp[1][i] = Q9465_bf[1][i] + std::max(Q9465_dp[0][i - 1], Q9465_dp[0][i - 2]);

	}

	return std::max(Q9465_dp[0][N - 1], Q9465_dp[1][N - 1]);	
}
